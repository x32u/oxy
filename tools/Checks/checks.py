...
def leveling_enabled():
   async def predicate(ctx: Context):
    ...
    await ctx.warn("Leveling is **not** enabled")
    return False
   return check(predicate)
  ...
  async def predicate(ctx: Context):
   if owner_id := await ctx.bot.db.fetchval("SELECT owner_id FROM antinuke WHERE guild_id = $1", ctx.guild.id):
    if ctx.author.id != owner_id: 
      await ctx.warn(f"Only <@!{owner_id}> can use this command!")
      return False 
    return True 
   await ctx.warn("Antinuke is **not** configured")
   return False
  return check(predicate)
  ...
  async def predicate(ctx: Context):
    ...
    if check: 
     ...
     if not ctx.author.id in allowed: 
      await ctx.warn("You **cannot** use this command")
      return False 
     return True 
    else: 
     await ctx.warn("Antinuke is **not** configured")
...
def br_is_configured():
  async def predicate(ctx: Context):
    ...
    if not check: 
      await ctx.warn("Booster roles are **not** configured")
...
def query_limit(table: str):
  async def predicate(ctx: Context):
    ...
    if check == 5: 
      await ctx.warn(f"You cannot create more than **5** {table} messages") 
      return False 
    return True 
  return check(predicate)
...
def boosted_to(level: int):
  async def predicate(ctx: Context):
   if ctx.guild.premium_tier < level: 
    await ctx.warn(f"The server has to be boosted to level **{level}** to be able to use this command")
...
def max_gws():
 async def predicate(ctx: Context):
  ...
  if check == 5:
   await ctx.warn("You cannot host more than **5** giveaways in the same time")
   return False
  return True
 return check(predicate)
...
def guild_owner():
  async def predicate(ctx: Context): 
    if ctx.author.id != ctx.guild.owner_id: 
     await ctx.warn(f"This command can be only used by **{ctx.guild.owner}**")
...
def antispam_enabled():
 async def predicate(ctx: Context):
  if not await ctx.bot.db.fetchrow("SELECT * FROM antispam WHERE guild_id = $1", ctx.guild.id):
    await ctx.warn("Antispam is **not** enabled in this server")
    return False 
  return True 
 return check(predicate)
...
def create_reskin():
 async def predicate(ctx: Context):
  if not await ctx.reskin_enabled():
   await ctx.warn("Reskin is **not** enabled in this server")
   return False 
...
def has_perks(): 
  async def predicate(ctx: Context): 
   ...
   if not check: 
    await ctx.warn(f"You need [**donator**](https://discord.gg/pretendbot) perks to use this command")   
   return check is not None
  return check(predicate)
...
def is_voice():
  async def predicate(ctx: Context): 
   if not ctx.author.voice: 
    await ctx.warn("You are not in a voice channel")
    return False
   if ctx.guild.me.voice: 
    if ctx.guild.me.voice.channel.id != ctx.author.voice.channel.id: 
     await ctx.warn("You are not in the same voice channel as the bot")
     return False
   return True 
  return check(predicate)  
...
def bot_is_voice():
 async def predicate(ctx: Context):   
  if not ctx.guild.me.voice: 
    await ctx.warn("The bot is not in a voice channel")
    return False
...
def lastfm_user_exists():
  async def predicate(ctx: Context):
   ...
   if not check: 
    await ctx.lastfm("You don't have a **Last.Fm** account set")
    return False 
   return True
  return check(predicate)
  ...
  if check: 
   if check['dice']:
    if datetime.datetime.now().timestamp() < check['dice']: 
     await ctx.utility(f"You can **dice** again **{ctx.bot.humanize_date(datetime.datetime.fromtimestamp(check['dice']))}**")
     return False 
  return True 
 return check(predicate)
...
def get_ticket():
 async def predicate(ctx: Context):  
  ...
  if check is None: 
   await ctx.warn("This message has to be used in an opened ticket")
   return False 
  return True 
 return check(predicate)
  ...
  if check: 
   ...
   if role: 
    if not role in ctx.author.roles and not ctx.author.guild_permissions.manage_channels: 
     await ctx.warn(f"Only members with {role.mention} role and members with the `manage_channels` permission can **add** or **remove** new members from the ticket")
     return False 
   else: 
    if not ctx.author.guild_permissions.manage_channels:
     await ctx.warn(f"Only members with the `manage_channels` permission can **add** or **remove** new members from the ticket")
     return False   
  else:
   if not ctx.author.guild_permissions.manage_channels:
     await ctx.warn(f"Only members with the `manage_channels` permission can **add** or **remove** new members from the ticket")
...
def is_there_a_reminder(): 
  async def predicate(ctx: Context):
    ...
    if not check: 
     await ctx.warn("You don't have a reminder set in this server")
    return check is not None
  return check(predicate)
...
def reminder_exists():
  async def predicate(ctx: Context):
   check = await ctx.bot.db.fetchrow("SELECT * FROM reminder WHERE guild_id = $1 AND user_id = $2", ctx.guild.id, ctx.author.id)
   if check:
    await ctx.warn("You already have a reminder in this channel")
    return False 
   return True
  return check(predicate)
...
def whitelist_enabled():
 async def predicate(ctx: Context):
  if not await ctx.bot.db.fetchrow(
   ...
   await ctx.warn(f"Whitelist is **not** enabled")
   return False
  return True
 return check(predicate)
